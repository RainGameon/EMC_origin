...
class QPLEX_curiosity_vdn_Learner_COIN:
    ...
    def update_memory(self, memory_buffer, mac, mixer, updata_batchsize):  # value propagation
        ...
        for i in range(memory_buffer.episodes_in_buffer - updata_batchsize):
            ...
            mac_out,_ = mac.forward(batch, batch.max_seq_length, batch_inf=True)  # [bs,T,n_agents,n_actions]
            ...
            target_mac_out,_ = self.target_mac.forward(batch, batch.max_seq_length,
    ...
    def sub_train(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac, mixer, optimiser, params,intrinsic_rewards,
        ...
        all_hidden_clone = all_hidden.clone()[:, :-1]
        # 如果需要保存缓冲区，那么保存
        if save_buffer:
            # 将内在奖励转换为NumPy数组并保存
        ...
        with th.no_grad():
            ...
            for i in range(self.args.n_agents):
                actions_one_hot_i_pre = th.cat((actions_one_hot_clone[:, :, :i], actions_one_hot_clone[:, :, i + 1:]),
                                               dim=2)  # (bs,T-1,n_agent-1,n_actions) ,a[t][-i]  得到除了当前 agent i 其他智能体对动作
                actions_one_hot_i = actions_one_hot_clone[:, :, i].unsqueeze(-2).repeat(1, 1, self.n_agents - 1,
                                                                                        1)  # (bs,T-1,n_agent-1,n_actions) ,a[t][i]   得到当前 agent i 动作
                obs_next_i = th.cat((obs_next_clone[:, :, :i], obs_next_clone[:, :, i + 1:]),
                                    dim=2)  # (bs,T-1,n_agent-1,obs_dim)    o[t+1][-i]得到除了当前 agent i 其他智能体下时刻的观测
                hidden_out_without_i = th.cat((all_hidden_clone[:, :, :i], all_hidden_clone[:, :, i + 1:]),dim=2)  # (bs,T-1,n_agent-1,hidden_dim) h[t+1][-i]得到除了当前 agent i 其他智能体下时刻的隐变量（表征了0～t+1的trajectory）

                influence_inputs_i_without_action = th.cat((hidden_out_without_i, actions_one_hot_i_pre), dim=-1)  # (bs,T-1,n_agent-1,hidden_dim+n_actions)  # 不考虑agent i 在时刻t的动作
                influence_inputs_i_with_action = th.cat( (hidden_out_without_i, actions_one_hot_i_pre, actions_one_hot_i),dim=-1)  # (bs,T-1,n_agent-1,hidden_dim+n_actions*2)  # 考虑了agent i 在时刻t的动作

                log_p_o_i = self.target_predict_without_action.get_log_pi(influence_inputs_i_without_action, obs_next_i)  # (bs,T-1,n_agent-1)

                log_q_o_i = self.target_predict_with_action.get_log_pi(influence_inputs_i_with_action,  obs_next_i)  # (bs,T-1,n_agent-1)
        ...
        if mixer is not None:
            ...
            if self.args.mixer == "dmaq_qatten":
                ans_chosen, q_attend_regs, head_entropies = \
                    mixer(chosen_action_qvals-influence_intrinsic_reward, batch["state"][:, :-1], is_v=True)                           # 计算V(s)
        ...
        # kl for vae
        q_normal = th.normal(0, 1, size=all_hidden.size()).to(all_hidden.device)
        Kl_hidden_out = 0.0001 * F.kl_div(th.log_softmax(all_hidden, -1), th.softmax(q_normal, -1), reduction='mean').sum()
        loss += Kl_hidden_out
        ...
        for i in range(self.args.n_agents):
            ...
            hidden_out_without_i = th.cat((all_hidden_clone[:, :, :i], all_hidden_clone[:, :, i + 1:]), dim=2).detach()
    ...
    def cuda(self):
        ...
        to_cuda(self.eval_predict_with_action, self.args.device)
        to_cuda(self.target_predict_with_action, self.args.device)
        to_cuda(self.eval_predict_without_action, self.args.device)
        to_cuda(self.target_predict_without_action, self.args.device)
        if self.mixer is not None:
