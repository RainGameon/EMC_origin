...
class vdn_QLearner:
    ...
    def subtrain(self, batch: EpisodeBatch, t_env: int, episode_num: int, mac,save_buffer=False, imac=None, timac=None):
        ...
        soft_target_mac_out= self.soft_target_mac.forward(batch, batch.max_seq_length, batch_inf=True)[0][:, 1:, ...]

        mac_out,_ = mac.forward(batch, batch.max_seq_length, batch_inf=True)
        predict_mac_out = self.predict_mac.forward(batch, batch.max_seq_length, batch_inf=True)[0][:, 1:, ...]
        ...
        if hasattr(self.args, 'use_qtotal_td') and self.args.use_qtotal_td: # false
